...
- 多结果集：连接时开启 `CLIENT_MULTI_STATEMENTS`，循环 `mysql_next_result`；单条语句出错时记录错误后继续处理后续结果集（是否继续由上层偏好决定）。
...
- 控制连接不可用（权限不足）时退化为 `mysql_kill`（`KILL CONNECTION`）：连接作废、标记需重连，并明确提示用户。
...
- 连接类错误（网络、认证、库不存在、SSL）与执行类错误（服务器返回的错误码）必须分开，UI 文案不能都是「连接失败」。
