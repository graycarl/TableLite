...
| S38 | 编辑器不支持用 `USE` 切换数据库 | 执行的语句默认落在侧栏当前选中的库上（切库时向服务器发一次 `USE` 同步）；脚本里手写 `USE xxx` 一律拦截、不下发，提示 `请通过侧栏切换数据库`。不做跨库脚本。见 `specs/06-query-editor.md` §3、`05-session-management.md` §11 |

## 2. 已知限制

| # | 限制 | 影响 | 缓解 |
| --- | --- | --- | --- |
| L1 | 结果集消费没有严格背压 | 超大结果集（百万行以上）时内存会增长 | 依赖限量查询（`LIMIT N`）与「结果较大，可随时停止」提示。若实际使用中成为问题，改为有界缓冲 + 阻塞式 `on_row` |
...
| L36 | 提交失败面板的「重试」只在事务状态已知（已回滚）时提供 | 事务超时 / `COMMIT` 失败时状态未知，重试可能重复写入 | 面板按 §5 显示「提交超时，事务状态未知，请在数据库中手动核对这几行的数据。」，此时只给「关闭并修正」 |
| L25 | 快速查看的 JSON 只 pretty-print、长文本无查找/行号 | 大 JSON 浏览不便 | 二进制 hex 与图片预览已做；按需再增强 |
| ~~L26~~ | ~~外键列的 ↗ 跳转未实现~~ | **已解决（2026-09-22）**：外键列非 NULL 单元格追加 `↗`，点击经 `ForeignKeyJumpResolver` 生成「引用列 = 原始值」条件并在新标签打开被引用表（跨库按元数据、复合外键按 AND）；截断值先二次加载，拿不到则提示并禁用。见 `specs/03` §1/§10、`docs/tech-designs/07-data-grid.md` §4/§9 | — |
...
| L37 | 导出面板的过滤行数估算用的是表数据视图的同一份估算（`information_schema.TABLES`，或用户点过「统计」后的过滤计数），不为此单独再跑一次 `COUNT(*)` | 过滤很强时可能高报行数 | 与网格下方条显示的估算保持一致；要精确可先在表数据视图点「统计」再导出 |
...
| 2026-09-22 | 导入导出接线与控件（第 6 批）：补齐查询结果标签右键「导出结果…」与网格右键「导出选中行…」两个入口；导出面板源说明显示真实过滤条件 + 行数估算；「后台导出，完成后通知我」接完成轻提示（含「在 Finder 中显示」）；导出进度进状态栏；导出「日期格式」改为下拉（原样 / 自定义，ENTRY 时直接作用于日期列）；导入「换行符」改为下拉、分隔符检测失败提示手动选择；导入第二步补「类型」列与下拉项文案；「跳过 / 未匹配」说明对齐 manual；导入进度补百分比；导入完成文案含「失败 0 行」并走轻提示。登记 L37–L39 |
| 2026-09-22 | 外键 `↗` 跳转（第 7 批，L26 关闭）：外键列非 NULL 单元格追加 `↗`，点击在新标签打开被引用表并带入「引用列 = 原始值」过滤；目标推导抽成纯函数 `ForeignKeyJumpResolver`（覆盖 NULL / 字符串转义 / 跨库 / 复合外键 / 二进制 hex），过滤条件以 Raw 模式复用行定位字面量生成，且不写回「按表记住的过滤」；截断值先二次加载，拿不到则提示并禁用 |
| 2026-09-23 | **表数据视图取消分页**：固定 `LIMIT N` 显示前 N 行（默认 300，可选 100/300/1000/5000 或手填，上限 10000），去掉页码 / 翻页 / 跳页与深分页提示（L11 消解，也不再做 `LIMIT N+1` 探测）；「精确统计」保留，点击后更新为精确总行数；无主键提示文案改为「行顺序不保证」；偏好「每页行数」改名「默认显示行数」。见 `specs/03-data-browsing.md` §2、`docs/tech-designs/07-data-grid.md` §7 |
| 2026-09-23 | **编辑器默认落在侧栏当前库**：切库 / 连接 / 重连时向服务器发一次 `USE` 同步默认库，切库失败回滚选择并提示；编辑器里手写 `USE` 一律拦截（提示 `请通过侧栏切换数据库`）。见 S38、L40、`specs/02-workspace.md` §4、`specs/06-query-editor.md` §3、`specs/09-readonly-mode.md` §4、`05-session-management.md` §11、`10-query-editor.md` §5.5 |
| 2026-09-23 | **表数据状态栏瘦身**：行数摘要与 `筛选` / `列` / `导出` 从窗口状态栏移到网格下方条（`GridStatusBarView`，原 `RowLimitBarView`），合并重复的行数信息；行数文案改为 `300 / 约 12,480 行`、显示条数下拉改为 `300 ▾`、`精确统计` 改为 `统计`；慢加载的耗时与「取消」也一并移到下方条。未提交改动提示条、提交进度、不可编辑原因仍留在状态栏（最小改动）。见 `specs/02-workspace.md` §7、`specs/03-data-browsing.md` §2、`specs/05-filtering.md` §1/§2、`specs/08-import-export.md`、`specs/12-feedback.md` §6、`docs/tech-designs/07-data-grid.md` §7 |
